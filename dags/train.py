...
from dags.config import TRAIN_DATA_PATH, MODEL_REGISTRY_FOLDER, DATA_FOLDER, MODEL_BOOSTING_REGISTRY_FOLDER, MODEL_BOOSTING_PATH
from formation_indus_ds_avancee.feature_engineering import prepare_features_with_io
from formation_indus_ds_avancee.train_and_predict import train_model_with_io, train_model_boosting_with_io
...
def train_model_boost():
    ...
    @task
    def train_model_boost_task(feature_path: str) -> None:
        train_model_boosting_with_io(features_path=feature_path, model_registry_folder=MODEL_BOOSTING_REGISTRY_FOLDER)

    feature_path = prepare_features_task()
    train_model_boost_task(feature_path)



train_model_dag = train_model()
train_model_boost_dag = train_model_boost()